#!/usr/bin/env python3
"""Agent creation and management for Boo-AutoAgent."""
...
def create_agent(
    ...
    **kwargs,
    ...
    agent_logger = logging.getLogger("BooAutoAgent")
    agent_logger.debug(
    ...
    try:
        if paths:
            ...
            if isinstance(root_path, str) and root_path:
                os.environ["BOO_OPERATION_ROOT"] = root_path
            if isinstance(artifacts_path, str) and artifacts_path:
                os.environ["BOO_ARTIFACTS_DIR"] = artifacts_path
            if isinstance(tools_path, str) and tools_path:
                os.environ["BOO_TOOLS_DIR"] = tools_path
            if operation_id:
                os.environ["BOO_OPERATION_ID"] = operation_id
            if target_name:
                os.environ["BOO_TARGET_NAME"] = target_name
    ...
    if has_existing_memories or config.memory_path:
        try:
            ...
            if memory_client:
                memory_overview = memory_client.get_memory_overview(user_id="boo_agent")
        except Exception as e:
    ...
    try:
        ...
        if module_tool_paths:
            ...
            try:
                agent_logger.info(
                    "BOOAUTOAGENT: module='%s', tools_discovered=%d, tools='%s'",
                    config.module,
                    len(tool_names),
                    ", ".join(tool_names),
                )
...
Boo Tools available in this environment:
...
"""
    ...
    try:
        ...
        agent_logger.info(
            "BOOAUTOAGENT: module='%s', execution_prompt_source='%s'",
            config.module,
            exec_src,
        )
    except Exception as e:
    ...
    try:
        ...
        if memory_client:
            active_plan = memory_client.get_active_plan(user_id="boo_agent")
            if active_plan:
    ...
    callback_handler = ReactBridgeHandler(
        ...
        init_context={
            ...
            "ui_mode": os.getenv("BOO_UI_MODE", "cli").lower(),
        },
    )
    ...
    agent_kwargs = {
        "model": model,
        "name": f"Boo-AutoAgent {config.op_id or operation_id}",
        "tools": tools_list,
        ...
        "trace_attributes": {
            ...
            "langfuse.user.id": f"boo-agent-{config.target}",
            ...
            "user.id": f"boo-agent-{config.target}",
            # Agent identification
            "agent.name": "Boo-AutoAgent",
            "agent.version": "1.0.0",
            "gen_ai.agent.name": "Boo-AutoAgent",
            "gen_ai.system": "Boo-AutoAgent",
            # Operation metadata
